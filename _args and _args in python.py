...
the value that we pass alongside it. That is why when we iterate over the
...
variable number of arguments to a function. It is used to pass a non-key worded,
...
can be tacked on to your current formal parameters (including zero extra
arguments).

For example, we want to make a multiply function that takes any number of
arguments and is able to multiply them all together. It can be done using *args.
